echo("################################################")
echo("[ToribashAI Upright V17] LUA LOADED - SMART STILLNESS")
echo("################################################")
...
local function load_agent()
    ...
    if not ok then
        echo("[Upright V17] AGENT LOAD ERROR")
        echo(tostring(err))
        return false
    end

    echo("################################################")
    echo("[Upright V17] AGENT RELOADED")
    echo("[Upright V17] agent = " .. tostring(TORIBASHAI_AGENT.name))
    echo("################################################")
...
local function write_result(score, x, y, z, shoulder_avg, head_avg, hip_avg, verticality_avg, facing_avg, step_avg, foot_spread_avg, recovery_avg, drift, fell, reason)
    ...
    f:write(string.format(
        ...
        hip_activity_sum, knee_activity_sum, arm_activity_sum, arm_reaction_frames, arm_success_frames, arm_failed_frames, arm_recovery_score_sum, shoulder_level_sum / math.max(posture_samples, 1), shoulder_unlevel_frames, calm_start_frames, panic_start_frames, stable_calm_start_frames, start_action_change_sum, early_motion_penalty_frames, stable_state_frames, smart_still_frames, useless_action_frames, perfect_stability_frames, unstable_action_frames, best_stable_streak, smart_still_score_sum,
...
local function finish_run(reason)
    ...
    echo("================================================")
    echo("[Upright V17] SCORE = " .. tostring(score))
    echo(string.format("[Upright V17] shoulder_high=%d head_high=%d hip_high=%d", shoulder_high_frames, head_high_frames, hip_high_frames))
    echo(string.format("[Upright V17] shoulder_avg=%.2f head_avg=%.2f hip_avg=%.2f", shoulder_avg, head_avg, hip_avg))
    echo(string.format("[Upright V17] verticality=%.2f drift=%.2f leg_activity=%.2f", verticality_avg, drift, leg_activity_sum))
    echo(string.format("[Upright V17] facing_avg=%.2f good=%d bad=%d turned=%d", facing_avg, facing_good_frames, facing_bad_frames, facing_turned_frames))
    echo(string.format("[Upright V17] step_avg=%.2f step_frames=%d bad_steps=%d foot_spread=%.2f", step_avg, step_recovery_frames, bad_step_frames, foot_spread_avg))
    echo(string.format("[Upright V17] recovery_avg=%.4f improve_sum=%.2f improve_frames=%d worse_frames=%d after_lean=%d lean_frames=%d", recovery_avg, recovery_improve_sum, recovery_improve_frames, recovery_worse_frames, recovery_after_lean_frames, lean_frames))
    echo(string.format("[Upright V17] hip_activity=%.2f knee_activity=%.2f", hip_activity_sum, knee_activity_sum))
    echo(string.format("[Upright V17] forward=%d early=%d fast=%d knee=%d hip=%d", forward_lean_frames, early_forward_lean_frames, fast_forward_lean_frames, forward_knee_reaction_frames, forward_hip_reaction_frames))
    echo(string.format("[Upright V17] early_knee=%d fast_knee=%d delayed=%d success_knee=%d failed=%d score=%.2f", early_knee_reaction_frames, fast_knee_reaction_frames, delayed_knee_penalty_frames, successful_knee_recovery_frames, failed_forward_recovery_frames, forward_recovery_score_sum))
    echo(string.format("[Upright V17] useful_reflexes=%d raw_reactions=%d", successful_knee_recovery_frames + successful_hip_recovery_frames, forward_knee_reaction_frames + forward_hip_reaction_frames + early_knee_reaction_frames + early_hip_reaction_frames + fast_knee_reaction_frames + fast_hip_reaction_frames))
    echo(string.format("[Upright V17] forward_crouch=%d hand_ground=%d best_hand_z=%.2f worst_crouch=%.2f", forward_crouch_frames, hand_ground_frames, best_hand_z, worst_forward_crouch))
    echo(string.format("[Upright V17] arms activity=%.2f reaction=%d success=%d failed=%d arm_score=%.2f", arm_activity_sum, arm_reaction_frames, arm_success_frames, arm_failed_frames, arm_recovery_score_sum))
    echo(string.format("[Upright V17] shoulders level_avg=%.2f unlevel=%d", shoulder_level_sum / math.max(posture_samples, 1), shoulder_unlevel_frames))
    echo(string.format("[Upright V17] upright_ratio=%.3f frame_bonus=%d", upright_ratio, frame * 12))
    echo("[Upright V17] facing disabled: rotation allowed for balance")
    echo(string.format("[Upright V17] calm_start=%d stable_calm=%d panic=%d early_motion_penalty=%d", calm_start_frames, stable_calm_start_frames, panic_start_frames, early_motion_penalty_frames))
    echo(string.format("[Upright V17] stable=%d smart_still=%d useless=%d perfect=%d best_streak=%d", stable_state_frames, smart_still_frames, useless_action_frames, perfect_stability_frames, best_stable_streak))
    echo("[Upright V17] fell=" .. tostring(fell) .. " reason=" .. tostring(reason))
    echo("================================================")
...
local function start_physics_once()
    ...
    echo("[Upright V17] STARTING PHYSICS")
...
remove_hooks("toribashai_upright_runner_v17")
remove_hooks("toribashai_upright_runner_v17")
remove_hooks("toribashai_upright_runner_v17")
remove_hooks("toribashai_upright_runner_v17")
remove_hooks("toribashai_upright_runner_v17")
...
add_hook("new_game", "toribashai_upright_runner_v17", on_new_game)
add_hook("draw2d", "toribashai_upright_runner_v17", on_draw2d)
add_hook("enter_frame", "toribashai_upright_runner_v17", on_enter_frame)
