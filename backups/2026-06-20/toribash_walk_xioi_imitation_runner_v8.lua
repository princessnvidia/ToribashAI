-- toribash_walk_xioi_imitation_runner_v8.lua
-- Branche walk_xioi_imitation V8
...
echo("################################################")
echo("[walk_xioi_imitation_v8] LUA LOADED - tolerant contact cuts")
echo("################################################")

local AGENT_PATH = "walk_xioi_imitation_champion_v1.json"
local REFERENCE_PATH = "xioi_reference_trajectory_v1.json"
local RESULT_PATH = "toribashai_episode_result_walk_xioi_imitation_v8.json"
...
local start_head_x = nil
local best_head_x = -999999
local best_foot_x = -999999
local max_foot_head_gap = 0
local alternating_step_score = 0
local same_time_leg_throw_frames = 0
local last_step_side = "none"
local step_switches = 0
local best_x = -999999
...
local function log(msg)
    echo("[walk_xioi_imitation_v8] " .. tostring(msg))
end
...
local function get_head_x()
    local x, _, _ = get_head()
    return x
end

local function get_feet_front_x()
    -- Plusieurs ids sont testés car les body ids Toribash varient selon le mod / version.
    -- Les appels invalides sont ignorés grâce à pcall.
    local front = nil
    for _, id in ipairs({10, 11, 12, 13}) do
        local ok, x, y, z = pcall(body_pos, PLAYER, id)
        if ok and x then
            if not front or x > front then
                front = x
            end
        end
    end
...
local function leg_side_from_pairs(pairs_for_frame)
    local right_count = 0
    local left_count = 0

    for _, pair in ipairs(pairs_for_frame) do
        if pair.joint == 14 or pair.joint == 16 or pair.joint == 18 then
            right_count = right_count + 1
        end
        if pair.joint == 15 or pair.joint == 17 or pair.joint == 19 then
            left_count = left_count + 1
        end
    end

    if right_count > 0 and left_count > 0 then
        return "both"
    end
...
local function update_step_alternation(pairs_for_frame)
    local side = leg_side_from_pairs(pairs_for_frame)

    if side == "both" then
        same_time_leg_throw_frames = same_time_leg_throw_frames + 1
        return
    end

    if side == "left" or side == "right" then
        if last_step_side ~= "none" and last_step_side ~= side then
            alternating_step_score = alternating_step_score + 1
            step_switches = step_switches + 1
        end
        last_step_side = side
    end
end
...
local function apply_pairs(command_frame)
    ...
    update_step_alternation(pairs_for_frame)
...
local function write_result(score, reason, x, y, z, forward, lateral_drift, avg_imitation_error)
    ...
    f:write(string.format(
        '{"score": %.4f, "reason": "%s", "frames": %d, "gen": "%s", "candidate": "%s", "population": "%s", "x": %.4f, "y": %.4f, "z": %.4f, "forward": %.4f, "head_forward": %.4f, "foot_head_gap": %.4f, "lateral_drift": %.4f, "best_head_x": %.4f, "best_foot_x": %.4f, "best_x": %.4f, "alternating_step_score": %d, "same_time_leg_throw_frames": %d, "step_switches": %d, "imitation_score_sum": %.4f, "imitation_frames": %d, "avg_imitation_error": %.4f, "best_imitation_frame_score": %.4f, "head_ground_frames": %d, "hip_ground_frames": %d, "shoulder_ground_frames": %d, "hand_ground_frames": %d, "torso_ground_frames": %d, "lateral_fall_frames": %d, "stalled_frames": %d}',
        score,
        tostring(reason),
        frame,
        ...
        forward or 0,
        forward or 0,
        max_foot_head_gap or 0,
        lateral_drift or 0,
        best_head_x,
        best_foot_x,
        best_x,
        alternating_step_score,
        same_time_leg_throw_frames,
        step_switches,
        imitation_score_sum,
        imitation_frames,
...
local function finish_run(reason)
    ...
    local head_x = get_head_x()
    local foot_x = get_feet_front_x()

    local forward = 0
    if head_x and start_head_x then
        forward = head_x - start_head_x
    else
        forward = x - (start_x or x)
    end

    local lateral_drift = math.abs(y - (start_y or y))

    if foot_x and head_x then
        local gap = foot_x - head_x
        if gap > max_foot_head_gap then
            max_foot_head_gap = gap
        end
    end
    ...
    score = score + frame * 3.0

    -- V6: la distance récompensée est celle de la tête, pas celle des pieds.
    score = score + forward * 650.0
    score = score + best_head_x * 120.0

    -- Récompense de marche: jambes alternées plutôt que les deux pieds jetés ensemble.
    score = score + alternating_step_score * 420.0
    score = score + step_switches * 180.0
    score = score - same_time_leg_throw_frames * 260.0

    -- Anti-triche: pieds projetés devant pendant que la tête reste derrière.
    if max_foot_head_gap > 1.6 then
        score = score - (max_foot_head_gap - 1.6) * 1400.0
    end

    score = score - avg_imitation_error * 520.0
    score = score - lateral_drift * 280.0
    ...
    if reason == "feet_thrown_forward" then score = score - 3000.0 end
    if reason == "both_feet_throw" then score = score - 2600.0 end
...
local function update_metrics(loop_frame)
    ...
    if head_x and not start_head_x then
        start_head_x = head_x
    end
    ...
    if head_x and head_x > best_head_x then best_head_x = head_x end
    if foot_x and foot_x > best_foot_x then best_foot_x = foot_x end
    ...
    -- V8: les seuils sol restent des pénalités, mais ne coupent plus la run.
    -- Ça évite les faux positifs où le script pense qu'une partie touche le sol
    -- alors que visuellement la marche est encore bonne.
    -- Les compteurs head/shoulder/torso/hip/hand_ground_frames sont toujours
    -- utilisés dans le score final.

    if frame > 100 and max_foot_head_gap > 3.4 then
        finish_run("feet_thrown_forward")
        return
    end

    -- V8: ne coupe que si les deux jambes sont vraiment spammées ensemble.
    if frame > 140 and same_time_leg_throw_frames > 45 and alternating_step_score < 3 then
        finish_run("both_feet_throw")
        return
    end
...
local function on_new_game()
    ...
    best_foot_x = -999999
    max_foot_head_gap = 0
    alternating_step_score = 0
    same_time_leg_throw_frames = 0
    last_step_side = "none"
    step_switches = 0
    best_x = -999999
...
local function on_draw2d()
    pcall(function()
        set_color(1, 1, 1, 1)
        draw_text("walk_xioi_imitation V8 | GEN " .. tostring(display_gen) .. " | CAND " .. tostring(display_candidate) .. "/" .. tostring(display_population), 20, 60, 0)
        draw_text("frame " .. tostring(frame) .. " | forward " .. string.format("%.2f", current_forward_display) .. " | head_forward " .. string.format("%.2f", current_forward_display) .. " | alt " .. tostring(alternating_step_score) .. " | both " .. tostring(same_time_leg_throw_frames) .. " | foot_gap " .. string.format("%.2f", max_foot_head_gap) .. " | imit_err " .. string.format("%.2f", current_error_display), 20, 82, 0)
    end)
end
...
remove_hooks("walk_xioi_imitation_runner_v8")
remove_hooks("walk_xioi_runner_v8")
remove_hooks("walk_xioi_runner_v7")
remove_hooks("walk_xioi_runner_v6")

add_hook("new_game", "walk_xioi_imitation_runner_v8", on_new_game)
add_hook("draw2d", "walk_xioi_imitation_runner_v8", on_draw2d)
add_hook("enter_frame", "walk_xioi_imitation_runner_v8", on_enter_frame)
