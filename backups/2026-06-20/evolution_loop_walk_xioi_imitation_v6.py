...
PROJECT_LUA = ROOT / "scripts/toribash_walk_xioi_imitation_runner_v6.lua"
TORIBASH_LUA = TORIBASH_SCRIPT_DIR / "toribash_walk_xioi_imitation_runner_v6.lua"
...
RESULT_PATH = TORIBASH_SCRIPT_DIR / "toribashai_episode_result_walk_xioi_imitation_v6.json"

POP_DIR = ROOT / "evolution/population_walk_xioi_imitation_v6"
BEST_DIR = ROOT / "evolution/best_walk_xioi_imitation_v6"
STATE_PATH = ROOT / "evolution/walk_xioi_imitation_v6_state.json"
...
def mutate(agent):
    ...
    child["mutation_strategy"] = "walk_xioi_imitation_v6_reference_trajectory_soft_mutation"
...
def reload_lua():
    send_chat_command("/ls toribash_walk_xioi_imitation_runner_v6.lua")
    time.sleep(0.5)
...
def evaluate(agent, gen, idx):
    RESULT_PATH.unlink(missing_ok=True)
    agent = normalize_agent(agent)
    agent["name"] = f"walk_xioi_imitation_v6_gen_{gen:05d}_agent_{idx:03d}"
    ...
    for gen in range(GENERATIONS):
        ...
        save_json(
            STATE_PATH,
            {
                ...
                "mutation_strategy": "walk_xioi_imitation_v6_reference_trajectory_soft_mutation",
