-- toribash_walk_xioi_imitation_runner_v6.lua
-- Branche walk_xioi_imitation V6
...
echo("################################################")
echo("[walk_xioi_imitation_v6] LUA LOADED - CACHE SAFE")
echo("################################################")

local AGENT_PATH = "walk_xioi_imitation_champion_v1.json"
local REFERENCE_PATH = "xioi_reference_trajectory_v1.json"
local RESULT_PATH = "toribashai_episode_result_walk_xioi_imitation_v6.json"
...
local function log(msg)
    echo("[walk_xioi_imitation_v6] " .. tostring(msg))
end
...
local function write_result(score, reason, x, y, z, forward, lateral_drift, avg_imitation_error)
    ...
    f:write(string.format(
        '{"score": %.4f, "reason": "%s", "frames": %d, "gen": "%s", "candidate": "%s", "population": "%s", "x": %.4f, "y": %.4f, "z": %.4f, "forward": %.4f, "head_forward": %.4f, "foot_head_gap": %.4f, "lateral_drift": %.4f, "best_head_x": %.4f, "best_foot_x": %.4f, "best_x": %.4f, "imitation_score_sum": %.4f, "imitation_frames": %d, "avg_imitation_error": %.4f, "best_imitation_frame_score": %.4f, "head_ground_frames": %d, "hip_ground_frames": %d, "shoulder_ground_frames": %d, "hand_ground_frames": %d, "torso_ground_frames": %d, "lateral_fall_frames": %d, "stalled_frames": %d}',
        score,
        tostring(reason),
        frame,
...
local function update_metrics(loop_frame)
    ...
    if frame > 70 and max_foot_head_gap > 2.8 then
...
local function on_draw2d()
    pcall(function()
        set_color(1, 1, 1, 1)
        draw_text("walk_xioi_imitation V6 | GEN " .. tostring(display_gen) .. " | CAND " .. tostring(display_candidate) .. "/" .. tostring(display_population), 20, 60, 0)
        draw_text("frame " .. tostring(frame) .. " | forward " .. string.format("%.2f", current_forward_display) .. " | head_forward " .. string.format("%.2f", current_forward_display) .. " | foot_gap " .. string.format("%.2f", max_foot_head_gap) .. " | imit_err " .. string.format("%.2f", current_error_display), 20, 82, 0)
    end)
end
...
remove_hooks("walk_xioi_imitation_runner_v6")
remove_hooks("walk_xioi_runner_v8")
remove_hooks("walk_xioi_runner_v7")
remove_hooks("walk_xioi_runner_v6")

add_hook("new_game", "walk_xioi_imitation_runner_v6", on_new_game)
add_hook("draw2d", "walk_xioi_imitation_runner_v6", on_draw2d)
add_hook("enter_frame", "walk_xioi_imitation_runner_v6", on_enter_frame)
