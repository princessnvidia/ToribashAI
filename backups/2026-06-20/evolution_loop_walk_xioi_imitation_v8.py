...
REFERENCE_PATH = ROOT / "evolution/xioi_reference_trajectory_v1.json"
...
PROJECT_LUA = ROOT / "scripts/toribash_walk_xioi_imitation_runner_v8.lua"
TORIBASH_LUA = TORIBASH_SCRIPT_DIR / "toribash_walk_xioi_imitation_runner_v8.lua"
TORIBASH_JSON = TORIBASH_SCRIPT_DIR / "walk_xioi_imitation_champion_v1.json"
TORIBASH_REFERENCE = TORIBASH_SCRIPT_DIR / "xioi_reference_trajectory_v1.json"
RESULT_PATH = TORIBASH_SCRIPT_DIR / "toribashai_episode_result_walk_xioi_imitation_v8.json"

POP_DIR = ROOT / "evolution/population_walk_xioi_imitation_v8"
BEST_DIR = ROOT / "evolution/best_walk_xioi_imitation_v8"
STATE_PATH = ROOT / "evolution/walk_xioi_imitation_v8_state.json"

POP_SIZE = 12
GENERATIONS = 999999

JOINT_VALUES = [1, 2, 3, 4]
MUTABLE_JOINTS = [14, 15, 16, 17, 18, 19]

# V3: imitation branch = very soft mutation.
BASE_MUTATION_RATE = 0.008
PAIR_ADD_RATE = 0.0015
PAIR_DROP_RATE = 0.001

EARLY_FRAME_PROTECTION = 120
EARLY_MUTATION_FACTOR = 0.05
...
def mutate(agent):
    ...
    for cmd in child["commands"]:
        ...
        for joint, state in cmd["pairs"]:
            if int(joint) not in MUTABLE_JOINTS:
        ...
        if random.random() < PAIR_ADD_RATE and frame >= EARLY_FRAME_PROTECTION:
            ...
            possible = [j for j in MUTABLE_JOINTS if j not in existing]
            if possible:
    ...
    if random.random() < 0.025:
        ...
        joint = random.choice(MUTABLE_JOINTS)
    ...
    child["mutation_strategy"] = "walk_xioi_imitation_v8_reference_trajectory_soft_mutation"

    return child


def copy_lua_and_reference_to_steam():
    if not PROJECT_LUA.exists():
        raise FileNotFoundError(PROJECT_LUA)

    if not REFERENCE_PATH.exists():
        raise FileNotFoundError(
            f"{REFERENCE_PATH} introuvable. Lance d'abord build_xioi_reference_trajectory.py"
        )

    shutil.copy2(PROJECT_LUA, TORIBASH_LUA)
    shutil.copy2(REFERENCE_PATH, TORIBASH_REFERENCE)

    print("Lua copié:", TORIBASH_LUA)
    print("Référence copiée:", TORIBASH_REFERENCE)
...
def reload_lua():
    send_chat_command("/ls toribash_walk_xioi_imitation_runner_v8.lua")
    time.sleep(0.5)
...
def evaluate(agent, gen, idx):
    RESULT_PATH.unlink(missing_ok=True)
    agent = normalize_agent(agent)
    agent["name"] = f"walk_xioi_imitation_v8_gen_{gen:05d}_agent_{idx:03d}"
    ...
    reset_toribash()
    # V6 head-forward: no /ls reload here. Toribash UI/autorun already loads the Lua.
    result = wait_result()
...
def main():
    ...
    ensure_seed()
    copy_lua_and_reference_to_steam()
    ...
    print()
    print("=" * 50)
    print("WALK XIOI IMITATION V8")
    print("=" * 50)
    print("V3 utilise xioi_reference_trajectory_v1.json pour noter la ressemblance posture par posture.")
    ...
    for gen in range(GENERATIONS):
        ...
        save_json(
            STATE_PATH,
            {
                ...
                "mutation_strategy": "walk_xioi_imitation_v8_reference_trajectory_soft_mutation",
