...
PROJECT_LUA = ROOT / "scripts/toribash_walk_xioi_imitation_runner_v5.lua"
TORIBASH_LUA = TORIBASH_SCRIPT_DIR / "toribash_walk_xioi_imitation_runner_v5.lua"
...
RESULT_PATH = TORIBASH_SCRIPT_DIR / "toribashai_episode_result_walk_xioi_imitation_v5.json"

POP_DIR = ROOT / "evolution/population_walk_xioi_imitation_v5"
BEST_DIR = ROOT / "evolution/best_walk_xioi_imitation_v5"
STATE_PATH = ROOT / "evolution/walk_xioi_imitation_v5_state.json"
...
def mutate(agent):
    ...
    child["mutation_strategy"] = "walk_xioi_imitation_v5_reference_trajectory_soft_mutation"
...
def reload_lua():
    send_chat_command("/ls toribash_walk_xioi_imitation_runner_v5.lua")
    time.sleep(0.5)
...
def evaluate(agent, gen, idx):
    RESULT_PATH.unlink(missing_ok=True)
    agent = normalize_agent(agent)
    agent["name"] = f"walk_xioi_imitation_v5_gen_{gen:05d}_agent_{idx:03d}"
    ...
    reset_toribash()
    # V5 cache-safe: no /ls reload here. Toribash UI/autorun already loads the Lua.
    result = wait_result()
    ...
    for gen in range(GENERATIONS):
        ...
        save_json(
            STATE_PATH,
            {
                ...
                "mutation_strategy": "walk_xioi_imitation_v5_reference_trajectory_soft_mutation",
