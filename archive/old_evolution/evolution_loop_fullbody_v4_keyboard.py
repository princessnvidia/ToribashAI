...
POP_DIR = ROOT / "evolution" / "population_fullbody_v4_keyboard"
BEST_DIR = ROOT / "evolution" / "best_fullbody_v4_keyboard"
...
def send_toribash_command(command):
    """
    Envoie une commande dans le chat Toribash via xdotool + clipboard.
    Version lente pour éviter les bugs t/reset et AZERTY.
    """

    run_shell(f'printf "{command}" | xclip -selection clipboard')
...
def init_toribash_manual():
    print()
    print("=================================================")
    print("INIT TORIBASH MANUELLE")
    print("=================================================")
    print("Dans Toribash, lance une seule fois :")
    print()
    print("/lm toribashai_goal_flat_v1.tbm")
    print("/ls toribash_reward_runner_v13.lua")
    print()
    print("Ensuite appuie sur Entrée ici.")
    input()


def reset_toribash():
    send_toribash_command("/reset")
...
def evaluate(agent_path):
    ...
    print("Reset automatique clavier lent")
...
def main():
    ...
    for gen in range(GENERATIONS):
        ...
        save_json(
            ROOT / "evolution" / "fullbody_v4_keyboard_last_summary.json",
            summary,
        )
