...
POP_DIR = ROOT / "evolution" / "population_fullbody_v1"
BEST_DIR = ROOT / "evolution" / "best_fullbody_v1"
...
def main():
    ...
    for gen in range(GENERATIONS):
        ...
        for i in range(POP_SIZE):
            ...
            path = (
                POP_DIR
                / f"gen_{gen:05d}_agent_{i:03d}.json"
            )
        ...
        if best_score > champion_score:
            ...
            champion_path = (
                BEST_DIR
                / (
                    f"champion_gen_{gen:05d}"
                    f"_score_{champion_score:.2f}.json"
                )
            )
            ...
            print()
            print(
                "Pas mieux que le champion actuel"
            )
