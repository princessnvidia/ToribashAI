echo("################################################")
echo("[ToribashAI V23] LUA LOADED - FULL AUTO FILE CONTROL")
echo("################################################")
...
local CONFIG = {
    ...
    control_path = "toribashai_control_v23.txt"
}
...
if not ok then
    echo("[ToribashAI V23] AGENT LOAD ERROR")
    echo(tostring(err))
    return false
    end

    echo("################################################")
    echo("[ToribashAI V23] AGENT RELOADED")
    echo("[ToribashAI V23] loaded agent = " .. tostring(TORIBASHAI_AGENT.name))
    echo("################################################")
    return true
    end
                        ...
                        for _, id in ipairs(ids) do
                            ...
                            if ok and x and y and z then
                                ...
                                if c == 0 then return nil end
                                    ...
                                    if not f then
                                        echo("[ToribashAI V23] ERROR: cannot write result file")
                                        return
                                        end

                                        f:write(string.format(
                                            '{"score": %.4f, "x": %.4f, "y": %.4f, "z": %.4f, "final_dist": %.4f, "progress_y": %.4f, "fell": %s, "frames": %d, "agent": "%s", "reason": "%s"}\n',
                                            score, x, y, z, final_dist, progress_y, tostring(fell), frame,
                                                              tostring(TORIBASHAI_AGENT.name), tostring(reason)
                                        ))

                                        f:close()
                                        echo("[ToribashAI V23] Result written: " .. CONFIG.result_path)
                                        end
                                                        ...
                                                        if not action then
                                                            ...
                                                            for i, joint_id in ipairs(AGENT_JOINTS) do
                                                                ...
                                                                if started_physics then return end
                                                                    ...
                                                                    echo("[ToribashAI V23] STARTING PHYSICS")
                                                                    unfreeze_game()
                                                                    ...
                                                                    if finished then return end
                                                                        ...
                                                                        if not x then
                                                                            echo("[ToribashAI V23] NIL POSITION ERROR")
                                                                            freeze_game()
                                                                            return
                                                                            end
                                                                            ...
                                                                            if start_y then progress_y = start_y - y end
                                                                                ...
                                                                                if fell then score = score - 150.0 end
                                                                                    if reason == "bad_progress" then score = score - 25.0 end
                                                                                        ...
                                                                                        echo("================================================")
                                                                                        echo("[ToribashAI V23] AGENT = " .. tostring(TORIBASHAI_AGENT.name))
                                                                                        echo(string.format("[ToribashAI V23] FINAL SCORE = %.2f", score))
                                                                                        echo(string.format("[ToribashAI V23] FINAL POS = %.2f %.2f %.2f", x, y, z))
                                                                                        echo(string.format("[ToribashAI V23] FINAL DIST = %.2f", final_dist))
                                                                                        echo(string.format("[ToribashAI V23] PROGRESS_Y = %.2f", progress_y))
                                                                                        echo("[ToribashAI V23] FELL = " .. tostring(fell))
                                                                                        echo("[ToribashAI V23] REASON = " .. tostring(reason))
                                                                                        echo("================================================")
                                                                                        ...
                                                                                        if not f then
                                                                                            ...
                                                                                            if not f then return end
                                                                                                ...
                                                                                                echo("[ToribashAI V23] CONTROL DETECTED: " .. tostring(cmd))
                                                                                                ...
                                                                                                if not reset_requested then return end
                                                                                                    ...
                                                                                                    if reset_delay > 0 then return end
                                                                                                        ...
                                                                                                        echo("[ToribashAI V23] RUN runCmd reset true")

                                                                                                        local ok, err = pcall(function()
                                                                                                        runCmd("reset", true)
                                                                                                        end)

                                                                                                        echo("[ToribashAI V23] reset ok = " .. tostring(ok))
                                                                                                        if not ok then
                                                                                                            echo("[ToribashAI V23] reset error = " .. tostring(err))
                                                                                                            end
                                                                                                            end
                                                                                                                                                ...
                                                                                                                                                if z < CONFIG.fall_z then
                                                                                                                                                    ...
                                                                                                                                                    if progress_y < CONFIG.min_progress_y then
                                                                                                                                                        ...
                                                                                                                                                        if frame >= CONFIG.max_frames then
                                                                                                                                                            ...
                                                                                                                                                            add_hook("new_game", "toribashai_reward_runner_v23", on_new_game)
                                                                                                                                                            add_hook("draw2d", "toribashai_reward_runner_v23", on_draw2d)
                                                                                                                                                            add_hook("enter_frame", "toribashai_reward_runner_v23", on_enter_frame)
