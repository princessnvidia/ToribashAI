...
MODELS = BASE / "models"
MODEL_PATH = MODELS / "parkour_mlp_v4_soft_weighted.pt"
SUMMARY_PATH = MODELS / "parkour_mlp_v4_soft_weighted_summary.json"
...
def prediction_distribution(logits):
    pred = logits.argmax(dim=-1).detach().cpu().reshape(-1).tolist()
    counts = Counter(pred)
    ...
    return {
        str(k): {
            ...
            "ratio": counts.get(k, 0) / total if total else 0.0,
        }
...
def eval_epoch(model, loader, criterion, device):
    ...
    if last_logits is not None:
        metrics["last_batch_prediction_distribution"] = prediction_distribution(last_logits)
...
def make_class_weights(class_counts, device):
    ...
    # V4 : pondération très douce.
    # 0.25 = racine quatrième de l'inverse frequency.
...
def main():
    ...
    print("Weight power:", WEIGHT_POWER)
