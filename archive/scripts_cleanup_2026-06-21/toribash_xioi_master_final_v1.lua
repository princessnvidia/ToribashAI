...
-- Lit xioi_master_final_v1_agent_current.lua (table Lua générée par Python),
-- applique les JOINT, ajoute un micro-pilote d'équilibre après frame 70,
-- puis écrit xioi_master_final_v1_result.json pour l'évolution.

local ACTION_FILE = "xioi_master_final_v1_agent_current.lua"
local RESULT_FILE = "xioi_master_final_v1_result.json"
local NAME = "xioi_master_final_v1"
...
local function balance_pilot(frame)
    -- Correction douce après frame 70 : épaules/bras et bassin/jambes opposés.
    -- Ne remplace pas l'action, ajoute seulement quelques contre-mouvements.
    ...
    local lean = ty - hy
    -- Si le buste part trop en avant/arrière sur Y, on contrebalance bras + hanches.
    if lean > 0.25 then
        -- buste trop +Y : bras opposés, hanches/jambes vers rattrapage
        set_joint(4, 2); set_joint(7, 4)
        set_joint(14, 2); set_joint(15, 3)
    elseif lean < -0.25 then
        set_joint(4, 4); set_joint(7, 2)
        set_joint(14, 3); set_joint(15, 2)
    end

    -- Si bassin trop bas, micro-extension jambes.
    if hz and tz and hz < tz - 2.0 then
        set_joint(16, 3); set_joint(17, 3); set_joint(18, 3); set_joint(19, 3)
    end
end
...
add_hook("enter_frame", NAME .. "_frame", function()
    ...
    if AUTORUN and run_frames and tick < MAX_TICKS then
...
add_hook("draw2d", NAME .. "_draw", function()
    if draw_text then
        draw_text("Xioi Master Final V1", 40, 60, 1)
        draw_text("loaded=" .. tostring(loaded) .. " err=" .. tostring(load_error), 40, 80, 1)
        draw_text("tick=" .. tostring(tick) .. " action=" .. tostring(last_action_frame) .. " pairs=" .. tostring(last_pairs), 40, 100, 1)
        draw_text("ok=" .. tostring(motor_ok) .. " fail=" .. tostring(motor_fail) .. " method=" .. tostring(last_method), 40, 120, 1)
        local dy = 0
        if start_y and last_y then dy = last_y - start_y end
        draw_text("Y torso dy=" .. string.format("%.3f", dy) .. " correct_from=" .. tostring(CORRECT_FROM_FRAME), 40, 140, 1)
    end
end)
