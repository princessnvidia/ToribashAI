#!/usr/bin/env python3
"""
evolution_xioi_master_final_v1.py

Xioi Master Final V1
...
Usage:
    ...
    python3 scripts/evolution_xioi_master_final_v1.py
"""
...
OUT_DIR = ROOT / "generated_replays" / "xioi_master_final_v1"
...
PROJECT_LUA = SCRIPTS / "toribash_xioi_master_final_v1.lua"
STEAM_LUA = TORIBASH_SCRIPT_DIR / "toribash_xioi_master_final_v1.lua"
STEAM_AGENT_LUA = TORIBASH_SCRIPT_DIR / "xioi_master_final_v1_agent_current.lua"
STEAM_RESULT = TORIBASH_SCRIPT_DIR / "xioi_master_final_v1_result.json"

CHAMPION_AGENT = OUT_DIR / "xioi_master_final_v1_champion_agent.json"
CHAMPION_RPL = OUT_DIR / "xioi_master_final_v1_champion.rpl"
HISTORY = OUT_DIR / "xioi_master_final_v1_history.jsonl"

# Parent préféré : champion promu V30, puis fallback mutation 23, puis source template.
PARENT_CANDIDATES = [
    OUT_DIR / "xioi_master_final_v1_champion.rpl",
...
# Mutations centrées après la frame 70.
PROTECTED_BEFORE = 70
EARLY_FOCUS_END = 180
MUTATE_RATE_MAIN = 0.09
MUTATE_RATE_LATE = 0.045
ADD_PAIR_RATE = 0.035
DROP_PAIR_RATE = 0.025
COUNTERSWING_RATE = 0.28
...
LUA_COMMAND = "/ls toribash_xioi_master_final_v1.lua"
RESET_COMMAND = "/reset"
...
def clean_steam_lua() -> None:
    ...
    keep = {
        ...
        "toribash_xioi_master_final_v1.lua",
        "xioi_master_final_v1_agent_current.lua",
    }
    removed = []
...
def find_parent_rpl() -> Path:
    ...
    if matches:
        return matches[0]
    raise FileNotFoundError("Aucun parent V30/V28 trouvé. Vérifie xioi_v30_23_mut.rpl ou le champion promu.")
...
def write_agent_lua(agent: dict[str, Any], path: Path) -> None:
    ...
    for key in ["generation", "candidate", "turnframes", "max_ticks", "correct_from_frame"]:
...
def weighted_frame_choice(frames: list[int]) -> int:
    ...
    # 70% entre 70 et 180, 20% 180-260, 10% après.
    r = random.random()
    early = [f for f in candidates if f <= EARLY_FOCUS_END]
    mid = [f for f in candidates if EARLY_FOCUS_END < f <= 260]
    late = [f for f in candidates if f > 260]
    if r < 0.70 and early:
...
def mutate_agent(parent_actions: list[dict[str, Any]], generation: int, candidate: int) -> dict[str, Any]:
    ...
    edits = random.randint(4, 9 + strength)

    for _ in range(edits):
        fr = weighted_frame_choice(frames)
        pairs = by_frame[fr]
        rate = MUTATE_RATE_MAIN if fr <= EARLY_FOCUS_END else MUTATE_RATE_LATE

        # mutation locale surtout sur épaules/bassin/jambes
        target_pool = BALANCE_JOINTS if random.random() < 0.82 else ALL_JOINTS
        if random.random() < 0.65 and pairs:
            existing = [j for j in pairs if j in target_pool] or list(pairs)
            j = random.choice(existing)
        else:
            j = random.choice(target_pool)

        if random.random() < DROP_PAIR_RATE and j in pairs and fr >= PROTECTED_BEFORE + 25:
            pairs.pop(j, None)
        else:
            old = pairs.get(j, random.choice(VALUES))
            if random.random() < rate * 3.0:
                new = random.choice([v for v in VALUES if v != old])
            else:
                new = clamp_value(old + random.choice([-1, 1]))
            pairs[j] = new

        # Ajouter parfois un couple bras/jambes opposés pour épaules/bassin.
        if random.random() < ADD_PAIR_RATE:
            pairs[random.choice(target_pool)] = random.choice(VALUES)

    # Contre-swing structuré après frame 70 : bras et jambes opposés.
...
def main() -> None:
    ...
    write_rpl_from_actions(header, best_agent["actions"], CHAMPION_RPL, "xioi_master_final_v1_champion")
