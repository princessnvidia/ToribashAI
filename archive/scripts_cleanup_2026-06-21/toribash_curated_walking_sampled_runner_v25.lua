-- toribash_curated_walking_sampled_runner_v25.lua
-- V25: runner live pour actions réelles sample depuis le dataset curated walking.
-- But: tester le runner + dataset sans GRU en boucle ouverte.

local HOOK = "curated_walking_sampled_v25"
local INPUT_FILE = "curated_walking_sampled_v25_live_actions_current.json"
...
local function safe_read_file(path)
    ...
    local ok, s = pcall(function() return f:read("*all") end)
    f:close()
...
local function draw_overlay()
    pcall(function()
        set_color(1, 1, 1, 1)
        draw_text("Curated Walking SAMPLED V25 AUTO", 40, 80, 1)
