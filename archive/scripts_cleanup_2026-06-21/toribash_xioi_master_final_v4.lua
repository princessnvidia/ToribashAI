...
-- Lit xioi_master_final_v4_agent_current.lua (table Lua générée par Python),
-- applique les JOINT, ajoute un micro-pilote d'équilibre après frame 70,
-- puis écrit xioi_master_final_v4_result.json pour l'évolution.

local ACTION_FILE = "xioi_master_final_v4_agent_current.lua"
local RESULT_FILE = "xioi_master_final_v4_result.json"
local NAME = "xioi_master_final_v4"
...
local function balance_pilot(frame)
    -- V4: très doux, uniquement après frame 70.
    -- Les 3 premiers pas viennent du champion V40_23; on ne les remplace pas.
    ...
    -- V4: correction beaucoup plus légère au début.
...
add_hook("new_game", NAME .. "_newgame", function()
    reset_state()
    -- V4: relance plus vite après reset si Toribash accepte run_frames.
...
add_hook("draw2d", NAME .. "_draw", function()
    if draw_text then
        draw_text("Xioi Master Final V4 gentle", 40, 60, 1)
