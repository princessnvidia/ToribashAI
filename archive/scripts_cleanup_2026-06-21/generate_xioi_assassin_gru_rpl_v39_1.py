#!/usr/bin/env python3
"""
generate_xioi_assassin_gru_rpl_v39_1.py
...
Outputs:
  generated_replays/xioi_assassin_gru_generated_v39_1.rpl
  generated_replays/xioi_assassin_gru_generated_v39_1_actions.json
  generated_replays/xioi_assassin_gru_generated_v39_1_summary.json
...
OUT_RPL = OUT_DIR / "xioi_assassin_gru_generated_v39_1.rpl"
OUT_ACTIONS = OUT_DIR / "xioi_assassin_gru_generated_v39_1_actions.json"
OUT_SUMMARY = OUT_DIR / "xioi_assassin_gru_generated_v39_1_summary.json"
STEAM_REPLAY_DIR = (
    Path.home()
...
def load_model(state_dim: int, device: torch.device) -> nn.Module:
    ...
    if isinstance(ckpt, dict):
        ...
        hidden = int(ckpt.get("hidden", ckpt.get("HIDDEN", 128)))
        layers = int(ckpt.get("layers", ckpt.get("LAYERS", 1)))
    else:
        state = ckpt
        hidden = 128
        layers = 1

    # Infer hidden/layers from actual checkpoint tensors when possible.
    if isinstance(state, dict):
        ...
        layer_ids = []
        for k in state.keys():
            m = re.search(r"gru\.weight_hh_l(\d+)", k)
            if m:
