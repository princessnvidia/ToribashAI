...
-- Lit xioi_master_final_v2_agent_current.lua (table Lua générée par Python),
-- applique les JOINT, ajoute un micro-pilote d'équilibre après frame 70,
-- puis écrit xioi_master_final_v2_result.json pour l'évolution.

local ACTION_FILE = "xioi_master_final_v2_agent_current.lua"
local RESULT_FILE = "xioi_master_final_v2_result.json"
local NAME = "xioi_master_final_v2"
...
local function balance_pilot(frame)
    -- V2: très doux, uniquement après frame 70.
    ...
    -- Micro-contrepoids épaules / bras opposés, seuil plus large que V1 pour éviter de casser la marche.
    if lean > 0.45 then
    ...
    if hz and tz and hz < tz - 2.6 then
...
add_hook("draw2d", NAME .. "_draw", function()
    if draw_text then
        draw_text("Xioi Master Final V2", 40, 60, 1)
