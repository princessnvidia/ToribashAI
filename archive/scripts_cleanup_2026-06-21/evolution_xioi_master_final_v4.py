#!/usr/bin/env python3
"""
evolution_xioi_master_final_v4.py

Xioi Master Final V4
- Mutations d’équilibre beaucoup plus douces entre frames 70-140.
- Auto-espace plus rapide après reset/load.
- Overlay Lua avec génération/candidat/population bien visible.
- Nettoie les anciens Lua Steam sauf toribash_upright_runner_v18.lua et toribash_recovery_runner_v1.lua.
- Part du champion V40 promu / xioi_v40_23_mut.rpl.
...
Usage:
    ...
    python3 scripts/evolution_xioi_master_final_v4.py
"""
...
OUT_DIR = ROOT / "generated_replays" / "xioi_master_final_v4"
...
PROJECT_LUA = SCRIPTS / "toribash_xioi_master_final_v4.lua"
STEAM_LUA = TORIBASH_SCRIPT_DIR / "toribash_xioi_master_final_v4.lua"
STEAM_AGENT_LUA = TORIBASH_SCRIPT_DIR / "xioi_master_final_v4_agent_current.lua"
STEAM_RESULT = TORIBASH_SCRIPT_DIR / "xioi_master_final_v4_result.json"

CHAMPION_AGENT = OUT_DIR / "xioi_master_final_v4_champion_agent.json"
CHAMPION_RPL = OUT_DIR / "xioi_master_final_v4_champion.rpl"
HISTORY = OUT_DIR / "xioi_master_final_v4_history.jsonl"

# Parent préféré : V4 champion si relance, sinon le V40_23 sain qui fait encore 3 pas.
PARENT_CANDIDATES = [
    OUT_DIR / "xioi_master_final_v4_champion.rpl",
    ROOT / "generated_replays" / "xioi_v40_23_mut.rpl",
    ROOT / "generated_replays" / "xioi_v40_champion.rpl",
...
# V4: on préserve exactement le démarrage qui marche déjà.
# 0-70 verrouillé. 70-140 micro-corrections. 140+ mutations légères.
PROTECTED_BEFORE = 70
MICRO_END = 140
EARLY_FOCUS_END = 220
MUTATE_RATE_MICRO = 0.0012
MUTATE_RATE_MAIN = 0.020
MUTATE_RATE_LATE = 0.016
ADD_PAIR_RATE = 0.004
DROP_PAIR_RATE = 0.002
COUNTERSWING_RATE = 0.07
...
MICRO_BALANCE_JOINTS = sorted(set([2, 3, 4, 5, 6, 7, 8, 9]))
ALL_JOINTS = list(range(20))
VALUES = [1, 2, 3, 4]

LUA_COMMAND = "/ls toribash_xioi_master_final_v4.lua"
RESET_COMMAND = "/reset"
...
def clean_steam_lua() -> None:
    ...
    keep = {
        ...
        "toribash_xioi_master_final_v4.lua",
        "xioi_master_final_v4_agent_current.lua",
        "toribash_upright_runner_v18.lua",
        "toribash_recovery_runner_v1.lua",
    }
    removed = []
...
def find_parent_rpl() -> Path:
    for p in PARENT_CANDIDATES:
        if p.exists():
            return p
    matches = sorted((ROOT / "generated_replays").glob("*v40*23*mut*.rpl"))
    if matches:
        return matches[0]
    raise FileNotFoundError("Aucun parent V40/V48 trouvé. Vérifie xioi_v40_23_mut.rpl ou le champion promu.")
...
def weighted_frame_choice(frames: list[int]) -> int:
    ...
    # V4: focus frames 70-220, avec surtout micro-corrections 70-140.
    r = random.random()
...
def mutate_agent(parent_actions: list[dict[str, Any]], generation: int, candidate: int) -> dict[str, Any]:
    ...
    strength = 1 + min(2, generation // 10)
    # V4: beaucoup moins d’éditions par candidat pour ne pas casser la marche.
    edits = random.randint(1, 2 + strength)
    ...
    for _ in range(edits):
        ...
        # V4: entre 70 et 140, on ne touche presque pas aux jambes: micro-équilibre seulement.
        if fr <= MICRO_END:
        ...
        if random.random() < DROP_PAIR_RATE and j in pairs and fr >= 150:
            ...
            if fr <= MICRO_END:
                # V4 ultra-doux: la marche de base survit, on ne fait que micro-orienter épaules/bassin.
                # 92% du temps on conserve exactement le joint. Sinon +1/-1 seulement.
        ...
        # Ajouter très rarement un couple, jamais dans la zone sensible 70-160.
        if random.random() < ADD_PAIR_RATE and fr >= 170:
            pairs[random.choice(target_pool)] = random.choice(VALUES)

    # Contre-swing structuré seulement tard, pour ne pas casser les 3 premiers pas.
    if random.random() < COUNTERSWING_RATE:
        late_frames = [f for f in frames if f >= 185]
...
def evaluate(agent: dict[str, Any], reload_lua: bool = False) -> dict[str, Any]:
    ...
    # V4: auto-start plus rapide juste après reset/load.
    press_space()
    time.sleep(0.035)
    press_space()
...
def main() -> None:
    ...
    write_rpl_from_actions(header, best_agent["actions"], CHAMPION_RPL, "xioi_master_final_v4_champion")
