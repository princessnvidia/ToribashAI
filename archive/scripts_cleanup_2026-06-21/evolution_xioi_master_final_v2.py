#!/usr/bin/env python3
"""
evolution_xioi_master_final_v2.py

Xioi Master Final V2
...
Usage:
    ...
    python3 scripts/evolution_xioi_master_final_v2.py
"""
...
OUT_DIR = ROOT / "generated_replays" / "xioi_master_final_v2"
...
PROJECT_LUA = SCRIPTS / "toribash_xioi_master_final_v2.lua"
STEAM_LUA = TORIBASH_SCRIPT_DIR / "toribash_xioi_master_final_v2.lua"
STEAM_AGENT_LUA = TORIBASH_SCRIPT_DIR / "xioi_master_final_v2_agent_current.lua"
STEAM_RESULT = TORIBASH_SCRIPT_DIR / "xioi_master_final_v2_result.json"

CHAMPION_AGENT = OUT_DIR / "xioi_master_final_v2_champion_agent.json"
CHAMPION_RPL = OUT_DIR / "xioi_master_final_v2_champion.rpl"
HISTORY = OUT_DIR / "xioi_master_final_v2_history.jsonl"

# Parent préféré : V2 champion si relance, sinon le V30_23 sain qui fait encore 3 pas.
PARENT_CANDIDATES = [
    OUT_DIR / "xioi_master_final_v2_champion.rpl",
...
# V2: on préserve exactement le démarrage qui marche déjà.
# 0-70 verrouillé. 70-140 micro-corrections. 140+ mutations légères.
PROTECTED_BEFORE = 70
MICRO_END = 140
EARLY_FOCUS_END = 220
MUTATE_RATE_MICRO = 0.025
MUTATE_RATE_MAIN = 0.045
MUTATE_RATE_LATE = 0.025
ADD_PAIR_RATE = 0.015
DROP_PAIR_RATE = 0.010
COUNTERSWING_RATE = 0.18
...
LUA_COMMAND = "/ls toribash_xioi_master_final_v2.lua"
RESET_COMMAND = "/reset"
...
def clean_steam_lua() -> None:
    ...
    keep = {
        ...
        "toribash_xioi_master_final_v2.lua",
        "xioi_master_final_v2_agent_current.lua",
        "toribash_upright_runner_v18.lua",
        "toribash_recovery_runner_v1.lua",
    }
    removed = []
...
def weighted_frame_choice(frames: list[int]) -> int:
    ...
    # V2: focus frames 70-220, avec surtout micro-corrections 70-140.
    r = random.random()
...
def mutate_agent(parent_actions: list[dict[str, Any]], generation: int, candidate: int) -> dict[str, Any]:
    ...
    edits = random.randint(2, 5 + strength)
    ...
    for _ in range(edits):
        ...
        if random.random() < DROP_PAIR_RATE and j in pairs and fr >= MICRO_END:
            pairs.pop(j, None)
        else:
        ...
        if random.random() < ADD_PAIR_RATE and fr >= 100:
...
def evaluate(agent: dict[str, Any], reload_lua: bool = False) -> dict[str, Any]:
    ...
    # V2: auto-start côté Python pour éviter le premier Espace manuel.
    press_space()
...
def main() -> None:
    ...
    write_rpl_from_actions(header, best_agent["actions"], CHAMPION_RPL, "xioi_master_final_v2_champion")
